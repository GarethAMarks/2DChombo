...
#if !defined(ROTATINGSOLUTION_HPP_)
#error "This file should only be included through RotatingSolution.hpp"
#endif

#ifndef ROTATINGSOLUTION_IMPL_HPP_
#define ROTATINGSOLUTION_IMPL_HPP_
...
RotatingSolution::RotatingSolution() {}

void RotatingSolution::initialise_from_file()
{
    // Read in the data files with the solution from Fortran
    std::ifstream id_file("initial_data.dat");

    if (!id_file.is_open()  ||
        !info_file.is_open())
    {
        std::cerr << "Error reading initial_data.dat!" << endl;
        exit(1);
    }

    // count the number of lines and x-vals per theta-val
    int line_count = 0;
    int x_count = 0;
    std::string line;
    ...
    pout() << "The file contains " << line_count << " lines." << endl;
...
double RotatingSolution::calculate_aspect_mass(double radius)
{
...
// Find the ADM mass
double RotatingSolution::calculate_adm_mass(double radius)
{
    double areal_radius = r_from_R_Spline(radius);
...
double RotatingSolution::calculate_radius(double dx)
{
...
double RotatingSolution::get_BSfrequency() const { return BS_frequency; }

void RotatingSolution::set_initialcondition_params(
...
#endif /* ROTATINGSOLUTION_IMPL_HPP_ */
