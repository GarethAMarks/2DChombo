...
void ThinShellSolution::initialise_from_file(std::string search_dir)
{
    // Read in the data files with the solution from Fortran
    std::ifstream A_file(search_dir + "/A.dat");
    std::ifstream m_file(search_dir +"/m.dat");
    std::ifstream phi_file(search_dir +"/Phi.dat");
    std::ifstream info_file(search_dir +"/output.dat");

    if (!A_file.is_open() || !phi_file.is_open() || !m_file.is_open() ||
        !info_file.is_open())
    {
        std::cerr << "Error reading thinshell files! Expected in directory " << search_dir << endl;
        exit(1);
    }
    ...
    while (std::getline(info_file, lineInfo))
    {
            ...
            lineInfo[0] != '#') // ignore leading commented lines
        {
            if (!(iss >> A_central >> omega_pre_rescale >> junk >>
                  ...
                  junk >> junk))
                std::cout
                    << "WARNING: reading thinshell output.dat may have failed; have BS_frequency =  " << BS_frequency 
                    << endl;
        }
    }
    ...
    double M = m_vals[line_count - 1];
    double R = int_r_vals[len_int_array - 1];
    double iso_scale_factor = (R - M - M *  M / (4. * (R -  M))) / (f_vals[len_int_array - 1] * R);
    ...
    {
        f_vals[j] *= f_max_arial_r * iso_scale_factor;  // / unscaled_f_at_max_r;
    ...
    Potential::params_t m_params_potential, const double max_r)
{
    ...
    mu = m_params_potential.scalar_mass;
