...
//Function to find the centres of the BSs using Gaussian Fitting performed via FitMRQ routine
...
double StarTracker::find_centre(int num_star, int direction)
{
	...
	{delta = m_width_A * (double(i) / double(m_points - 1) - 0.5);}
        ...
        if (direction == 0 )
        {
            x_coords[i] = m_star_coords[2 * num_star] + delta;
            y_coords[i] = m_star_coords[2 * num_star + 1];
        }

        if (direction == 1 )
        {	
            x_coords[i] = m_star_coords[2 * num_star];
            y_coords[i] = m_star_coords[2 * num_star + 1] + delta;
        }
    ...
    a_vector[1] = m_star_coords[2 * num_star];
	if (num_star == 0)
    ...
    {	
	a_vector[0] = vals_f[(m_points-1)/2];
    a_vector[1] = m_star_coords[2 * num_star + 1];
	if (num_star == 0)
        {a_vector[2] = m_width_A/2;}
...
void StarTracker::find_max_min(int num_star, int direction)
{
    ...
    if (direction == 0)
    {
        ...
        m_star_coords[2 * num_star] = sum1 / sum2;
    }
    ...
    if (direction == 1)
    {
        ...
        m_star_coords[2 * num_star + 1] = sum1 / sum2;
    }
                               ...
                               bool a_first_step)
{
    ...
    std::vector<string> header_line(2. * m_num_stars);

    for (int n = 0; n < m_num_stars; n++)
    {
        header_line[2 * n] = "x" + to_string(n+1);
        header_line[2 * n + 1] = "y" + to_string(n+1);
    }
...
{
    // Positions already initialised via initial_setup().
    // Restart-from-file support requires time/dt info; seed from param file for now.
}

void StarTracker::execute_tracking(double a_time, double a_restart_time,
                                   double a_dt, bool a_write)
{
    update_star_centres(a_dt);
    if (a_write)
    {
        bool first_step = (a_time == 0.);
        write_to_dat("StarCentres", a_dt, a_time, a_restart_time, first_step);
    }
}
